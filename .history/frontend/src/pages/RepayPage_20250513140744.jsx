    ...
    const fetchAll = async () => {
      ...
      try {
        const [balanceRes, healthRes, previewHealthRes, previewDebtRes] = await Promise.all([
          ...
          fetch(`http://localhost:3001/api/previewhealthfactorrepay?userAddress=${account}&assetAddress=${assetAddress}&repayAmount=${amount}`),
          fetch(`http://localhost:3001/api/previewRemaingDebt?userAddress=${account}&assetAddress=${assetAddress}&repayAmount=${amount}`)
        ]);
