    ...
    const fetchMetadata = async () => {
      try {
        ...
        for (const token of data) {
          parsedMetadata[token.address] = {
            symbol: token.symbol,
            icon: tokenSymbols[token.symbol] || null,
          };
        }
  ...
  const applyFilters = () => {
    ...
    if (token)
      result = result.filter(
        (tx) => tokenMetadata[tx.token]?.symbol.toLowerCase() === token.toLowerCase()
      );
    if (amount)
          ...
          <table className="tx-table">
            ...
            <tbody className="tx-table-body">
              {filtered.map((tx, index) => {
                const meta = tokenMetadata[tx.token];
