...
const LendingController = {
  async deposit(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Process failed", details: err.message });
    }
  },
  ...
  async getLenderBalance(req, res) {
    try {
      ...
      const accrueTx = await LendingPoolContract.methods
        .accrueInterest(assetAddress)
    ...
    try {
      ...
      return res.status(200).json({
        ...
        config: {
          ...
          liquidationThreshold:
            (Number(liquidationThreshold) / 1000).toFixed(2) + "%",
          liquidationPenalty:
            (Number(liquidationPenalty) / 10).toFixed(2) + "%",
          reserveFactor:
            (Number(reserveFactor.reserveFactor) / 100).toFixed(2) + "%",
        },
      });
    } catch (err) {
      return res.status(500).json({
  ...
  async withdraw(req, res) {
    try {
      ...
      if (!coingeckoMap[tokenKey]) {
        return res
          .status(400)
          .json({ error: `Symbol ${symbol} is not mapped to a price feed` });
      }
      ...
      if (!priceUSD) {
        return res
          .status(500)
          .json({ error: "Unable to fetch price for token" });
      }

      const balanceHuman = Number(ethers.formatUnits(currentBalance, decimals));
      const debtRaw = await LendingPoolContract.methods
      ...
      if (Number(amount) > maxWithdrawAmount) {
        return res.status(400).json({
          ...
          priceUSD,
          liquidationThreshold:
            (Number(liquidationThresholdBP) / 1000).toFixed(2) + "%",
      ...
      return res
        .status(500)
        .json({ error: "Withdrawal failed", details: err.message });
    }
  },
  ...
  async getTotalSuppliedAndBorrow(req, res) {
    ...
    try {
      ...
      const [tokenState, supplyCapRaw, borrowCapRaw, prices] =
        await Promise.all([
      ...
      const totalSupplied = parseFloat(
        ethers.formatUnits(tokenState.totalDeposits, decimals)
      );
      const totalBorrowed = parseFloat(
        ethers.formatUnits(tokenState.totalBorrows, decimals)
      );
      ...
      const supplyPercent =
        supplyCap === 0
          ? "0.00"
          : ((totalSupplied / supplyCap) * 100).toFixed(2);
      const borrowPercentOfSupply =
        totalSupplied === 0
          ? "0.00"
          : ((totalBorrowed / totalSupplied) * 100).toFixed(2);
  ...
  async getUtilizationRate(req, res) {
    try {
      ...
      const rate = await LendingPoolContract.methods
        .getUtilization(assetAddress)
        .call();
  ...
  async getAvailableLiquidity(req, res) {
    try {
      ...
      const tokenState = await LendingPoolContract.methods
        .tokenState(assetAddress)
        .call();
      const totalDeposits = BigInt(tokenState.totalDeposits);
      const totalBorrows = BigInt(tokenState.totalBorrows);
      const available =
        totalDeposits > totalBorrows ? totalDeposits - totalBorrows : 0n;
      return res.status(200).json({
        asset: assetAddress,
  ...
  async getSupplyAPYandUtilization(req, res) {
    try {
      const supportedTokens = await LendingPoolContract.methods
        .getSupportedTokens()
        .call();
      ...
      for (const token of supportedTokens) {
        try {
          const utilization = await LendingPoolContract.methods
            ...
            .call();
          const supplyAPY = await InterestModel.methods
            .getSupplyAPY(token, utilization)
            .call();
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch supply APYs", details: err.message });
    }
  },
        ...
        supportedTokens.map(async (token) => {
          try {
            const [utilization, supplyAPY] = await Promise.all([
              ...
              InterestModel.methods
                .getSupplyAPY(
                  token,
                  await LendingPoolContract.methods.getUtilization(token).call()
                )
                .call(),
            ]);
  ...
  async getUserHistory(req, res) {
    ...
    try {
      const [depositEvents, withdrawEvents, borrowEvents, repayEvents] =
        await Promise.all([
      ...
      let allEvents = [
        ...
        amount: ethers.formatUnits(
          e.returnValues.amount.toString(),
          DEFAULT_DECIMALS
        ),
  ...
  async getLenderCollateral(req, res) {
    try {
      ...
      for (let i = 0; i < tokenAddresses.length; i++) {
        ...
        try {
          await LendingPoolContract.methods
            .accrueInterest(tokenAddress)
            .send({ from: userAddress });
        } catch (accrueErr) {
          console.warn(
            `Failed to accrue interest for ${tokenAddress}:`,
            accrueErr.message
          );
          continue;
        }
      ...
      return res.status(500).json({
        error: "Failed to fetch user collateral",
        details: err.message,
      });
    }
  },
  ...
  async SumAllCollateral(req, res) {
    try {
      ...
      const { 0: tokenAddresses, 1: balances } =
        await LendingPoolContract.methods.getUserCollateral(userAddress).call();

      const tokensToProcess = tokenAddresses.filter(
        (_, i) => balances[i] !== "0"
      );
      ...
      if (tokensToProcess.length === 0) {
        return res
          .status(200)
          .json({ user: userAddress, totalCollateralUSD: "0.00" });
      }
      ...
      const tokenInfos = await Promise.all(
        tokensToProcess.map(async (token, i) => {
          ...
          const userBalance = parseFloat(
            ethers.formatUnits(rawBalance.toString(), decimals)
          );
  ...
  async TotalAPY(req, res) {
    try {
      ...
      if (!userAddress || !web3.utils.isAddress(userAddress)) {
        return res
          .status(400)
          .json({ error: "Invalid or missing user address" });
      }

      const totalAPYRaw = await LendingPoolContract.methods
  ...
  async claimAllTokensForUser(req, res) {
    try {
      ...
      for (const [tokenAddress, faucetAddress] of Object.entries(faucetMap)) {
        ...
        const alreadyClaimed = await faucetContract.methods
        ...
        try {
          const gasEstimate = await faucetContract.methods
            .claimTokens()
  ...
  async getMaxWithdrawable(req, res) {
    try {
      ...
      if (!priceUSD || isNaN(priceUSD)) {
        return res
          .status(500)
          .json({ error: "Unable to fetch valid price for token" });
      }

      // Simulate interest accrual for this asset
      await LendingPoolContract.methods
        .accrueBorrowInterest(assetAddress)
        .call();
      ...
      if (borrowedUSD === 0) {
        ...
        availableUSD =
          balanceUSD - borrowedUSD / (Number(liquidationThresholdBP) / 100000);
  ...
  async setAssetConfig(req, res) {
    try {
      ...
      if (liquidationPenalty > 2000) {
        return res
          .status(400)
          .json({ error: "Liquidation penalty cannot exceed 20%" });
      }
  ...
  async borrow(req, res) {
    try {
      ...
      const [supportedTokens, tokenContract, collateral, maxLTVBP, tokenState] =
        await Promise.all([
      ...
      const currentDebtRaw = await LendingPoolContract.methods
        .repayBalanceOf(assetAddress, fromAddress)
        .call();
      const userDebt = parseFloat(
        ethers.formatUnits(currentDebtRaw.toString(), DEFAULT_DECIMALS)
      );
  ...
  async repay(req, res) {
    try {
      ...
      if (BigInt(allowance) < BigInt(amountInSmallestUnit)) {
        try {
          console.log(
            "[Repay] Insufficient allowance, attempting to approve..."
          );
          const gasEstimate = await tokenContract.methods
            .approve(LendingPoolContract.options.address, amountInSmallestUnit)
            .estimateGas({ from: fromAddress });
      ...
      const tx = await LendingPoolContract.methods
        ...
        .send({ from: fromAddress, gas: Math.ceil(Number(gasEstimate) * 1.5) });
      console.log(
        "[Repay] Repayment transaction successful:",
        tx.transactionHash
      );
  ...
  async getHealthFactor(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch health factor", details: err.message });
    }
  },
  ...
  async getMaxBorrowable(req, res) {
    try {
      ...
      const tokenContract = getTokenContract(assetAddress);
      const [symbol, decimalsRaw, maxLTVBPRaw, borrowCapRaw, tokenState] =
        await Promise.all([
          tokenContract.methods.symbol().call(),
          tokenContract.methods.decimals().call(),
          LendingPoolContract.methods.maxLTV(assetAddress).call(),
      ...
      const debtRaw = await LendingPoolContract.methods
        .repayBalanceOf(assetAddress, userAddress)
        .call();
      const userBorrowBalance = parseFloat(
        ethers.formatUnits(debtRaw.toString(), decimals)
      );
  ...
  async getSupplyAPR(req, res) {
    try {
      ...
      if (tokenAddress) {
        ...
        tokensToQuery = await LendingPoolContract.methods
          .getSupportedTokens()
          .call();
      }
      ...
      for (const token of supportedTokens) {
        try {
          const utilization = await LendingPoolContract.methods
            .getUtilization(token)
            .call();
          const borrowAPY = await InterestModel.methods
            .getBorrowAPY(token, utilization)
            .call();
  ...
  async getBorrowAPR(req, res) {
    try {
      ...
      if (!tokenAddress || !ethers.isAddress(tokenAddress)) {
        return res
          .status(400)
          .json({ error: "Invalid or missing token address" });
      }

      const utilization = await LendingPoolContract.methods
        .getUtilization(tokenAddress)
        .call();
      const borrowAPR = await InterestModel.methods
        .getBorrowRate(tokenAddress, utilization)
        .call();
  ...
  async PreviewCollateralAfterWithdraw(req, res) {
    try {
      ...
      if (tokenIndex === -1) {
        return res
          .status(400)
          .json({ error: "Token not found in user collateral" });
      }
      ...
      const decimals = Number(decimalsRaw);
      const withdrawAmountBig = ethers.parseUnits(
      ...
      return res.status(200).json({
        ...
        asset: assetAddress,
        symbol,
        originalCollateral: ethers.formatUnits(
  ...
  async PreviewRemainingDebtAfterRepay(req, res) {
    try {
      ...
      const decimals = Number(decimalsRaw);
      const repayAmountBig = ethers.parseUnits(
  ...
  async TotalBorrowAPY(req, res) {
    try {
      ...
      for (let i = 0; i < tokenAddresses.length; i++) {
        ...
        const borrowAPYbps = await InterestModel.methods
          .getBorrowAPY(token, utilization)
          .call();
        ...
        const userBorrow = parseFloat(
          ethers.formatUnits(rawBorrow, Number(decimals))
        );
      ...
      const totalBorrowAPY =
        totalBorrowUSD > 0 ? weightedAPYSum / totalBorrowUSD : 0;
      ...
      return res.status(500).json({
        error: "Failed to calculate total borrow APY",
        details: err.message,
      });
    }
  },
  async SumAllBorrow(req, res) {
    try {
      ...
      const { 0: tokenAddresses, 1: rawPrincipals } =
        await LendingPoolContract.methods.getUserBorrow(userAddress).call();

      const tokensToProcess = tokenAddresses.filter(
        (_, i) => rawPrincipals[i] !== "0"
      );

      if (tokensToProcess.length === 0) {
        return res
          .status(200)
          .json({ user: userAddress, totalBorrowUSD: "0.00" });
      }
      ...
      const tokenInfos = await Promise.all(
        tokensToProcess.map(async (token) => {
          ...
          const debtAmount = parseFloat(
            ethers.formatUnits(rawDebt.toString(), decimals)
          );
      ...
      const debt = await Promise.all(
        tokenAddresses.map(async (tokenAddress) => {
          try {
            // Accrue borrow interest first
            await LendingPoolContract.methods
              .accrueBorrowInterest(tokenAddress)
              .send({ from: userAddress });
            ...
            console.error(
              `Failed to fetch debt for ${tokenAddress}:`,
              innerErr.message
            );
            return {
              tokenAddress,
  ...
  async getAssetOverview(req, res) {
    try {
      ...
      for (const tokenAddress of tokens) {
        ...
        const [symbol, decimals, rawWalletBalance, canBeCollateral] =
          await Promise.all([
            tokenC.methods.symbol().call(),
            tokenC.methods.decimals().call(),
    ...
    try {
      ...
      const metas = await Promise.all(
        tokens.map(async (tokenAddress) => {
          const tokenC = getTokenContract(tokenAddress);
          const [symbol, decRaw, maxLTVbpRaw, rawWalletBalance] =
            await Promise.all([
              tokenC.methods.symbol().call(),
              tokenC.methods.decimals().call(),
  ...
  async getTotalBorrowed(req, res) {
    ...
    try {
      ...
      const borrowPercent = (
        (parseFloat(totalBorrowed) / parseFloat(maxBorrow)) *
        100
      ).toFixed(2);
  ...
  async getUserTotalDebtUSD(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch total debt", details: err.message });
    }
  },
    ...
    try {
      ...
      for (const assetAddress of supportedTokens) {
        try {
          ...
          console.error(
            `Error fetching data for token ${assetAddress}:`,
            innerErr.message
          );
        }
      }
      ...
      console.error(
        "Error fetching total supplied and borrowed for all assets:",
        err.message
      );
      return res.status(500).json({
  ...
  async PreviewHealthFactorBorrow(req, res) {
    try {
      ...
      const borrowAmountInSmallestUnit = ethers.parseUnits(
      ...
      const tokenPricesUSDInSmallestUnit = tokenPricesUSD.map((price) => {
        const priceStr = price.toLocaleString("fullwide", {
          useGrouping: false,
        });
  ...
  async PreviewHealthFactorRepay(req, res) {
    try {
      ...
      const repayAmountInSmallestUnit = ethers.parseUnits(
        repayAmount.toString(),
        18
      );
      ...
      const healthFactor = await LendingPoolContract.methods
        .previewHealthFactorAfterRepay(
          userAddress,
          assetAddress,
          repayAmountInSmallestUnit,
          tokenPricesUSDInSmallestUnit
        )
        .call();
  ...
  async checkCapsStatus(req, res) {
    try {
      ...
      const results = await Promise.all(
        supportedTokens.map(async (assetAddress) => {
          try {
