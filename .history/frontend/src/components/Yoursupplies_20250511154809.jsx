  ...
  useEffect(() => {
    ...
    const fetchSupplyStats = async () => {
      try {
        ...
        setSupplyStats({
          balance: `$${parseFloat(collateral.totalCollateralUSD).toFixed(2)}`,
          apy: `${parseFloat(apy.totalAPY).toFixed(2)}%`,
          collateral: `$${parseFloat(balance.totalCollateralTokenSum).toFixed(2)}`,
        });
      } catch (err) {
    ...
    const fetchWalletBalances = async () => {
      try {
        ...
        const filtered = (data.balances || []).filter(asset => parseFloat(asset.walletBalance) > 0);
            ...
            <tbody style={{ textAlign: 'left' }}>
              {walletBalances.map((asset, index) => (
                <tr key={index}>
                  ...
                  <td>{parseFloat(asset.walletBalance).toFixed(2)}</td>
