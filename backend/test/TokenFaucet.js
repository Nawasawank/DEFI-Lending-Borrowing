...
describe("TokenFaucet", function () {
  ...
  beforeEach(async function () {
    ...
    token = await Token.deploy("Test Token", "TTK", parseUnits("1000", 18));
  ...
  it("should allow different users to claim individually", async function () {
    ...
    expect(balance1).to.equal(parseUnits("100", 18));
    expect(balance2).to.equal(parseUnits("100", 18));
  });

  it("should revert if faucet has not been set in token", async function () {
    // Deploy a new token without setting faucet
    const token2 = await Token.deploy("Test Token", "TTK", parseUnits("1000", 18));
