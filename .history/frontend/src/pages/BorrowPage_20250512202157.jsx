...
const BorrowPage = ({ onClose, tokenName, apy }) => {
  const [amount, setAmount] = useState("10"); // Use string here
  ...
  useEffect(() => {
    const fetchHealthFactor = async () => {
      ...
      const assetAddress = tokenAddresses[tokenName];
      try {
        const res = await fetch(`http://localhost:3001/api/previewhealthfactorborrow?userAddress=${account}&assetAddress=${assetAddress}&borrowAmount=${numericAmount}`);
        const data = await res.json();
