  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        setNetOverview({
          netWorthUSD: parseFloat(netData.netWorthUSD).toFixed(2),
          netAPY: parseFloat(netData.netAPY).toFixed(2) + '%',
          healthFactor: parseFloat(healthData.healthFactor).toFixed(2),
        });
      } catch (err) {
    ...
    <div className="icons-container">
      <div className="icon-group">
        ...
        <div className="icon-box">
          ...
          <span className="icon-value">{netOverview.healthFactor}</span>
        </div>
      </div>
