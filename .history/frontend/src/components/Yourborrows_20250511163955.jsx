...
function Yourborrows() {
  ...
  useEffect(() => {
    const fetchBorrowStats = async () => {
      if (!account) return;
      try {
        const res = await fetch(`http://localhost:3001/api/total-borrow?userAddress=${account}`);
        const data = await res.json();

        setBorrowStats({
          balance: '$0.00', // placeholder if you want to calculate available balance
          apy: '3.45%',     // example static or calculated later
          totalDebt: `$${parseFloat(data.totalBorrowUSD).toFixed(2)}`,
        });
      } catch (err) {
        ...
        <div className="borrows">
          <div className="borrows-header">
            ...
            <div className="info-sectionb">
              <div className="info-boxb">Balance {borrowStats.balance}</div>
