...
function Yourborrows() {
  ...
  useEffect(() => {
    const fetchBorrowStats = async () => {
      if (!account) return;
      try {
        const res = await fetch(`http://localhost:3001/api/sumBorrow?userAddress=${account}`);
        const data = await res.json();

        const formattedValue = `$${parseFloat(data.totalBorrowUSD).toFixed(2)}`;
        setBorrowStats({
        ...
        <div className="borrows">
          <div className="borrows-header">
            ...
            <div className="info-sectionb">
              <div className="info-boxb">Balance {borrowStats.totalDebt}</div>
