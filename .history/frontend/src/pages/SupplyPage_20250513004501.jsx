...
const SupplyPage = ({ onClose, tokenName, amount }) => {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      if (!account) return;
      try {
        const res = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/check-status?userAddress=${account}`);
        const data = await res.json();
        const map = {};
        data.forEach(item => {
          map[item.asset.toLowerCase()] = item.isSupplyFull;
        });
        setSupplyStatusMap(map);
      } catch (err) {
        console.error("Failed to fetch supply status:", err);
      }
    };
    fetchStatus();
  }, [account]);
  ...
  const isSupplyFull = supplyStatusMap[assetAddress];

  const handleSupplyClick = async () => {
    if (inputAmount <= 0 || inputAmount > walletBalance || isSupplyFull) {
      setHasError(true);
